...
public class RecJavaUtils {
    ...
    public static Size getSizeFromSps(byte[] data) {
        for (int i = 0; i < data.length - 4; i++) {
            if (data[i] == 0 && data[i + 1] == 0 && data[i + 2] == 0 && data[i + 3] == 1 && data[i + 4] == 0x67) {
                int[] sizeArray = new int[2];
                h264_decode_seq_parameter_set(data, data.length, sizeArray);
                return new Size(sizeArray[0], sizeArray[1]);
            }
        }
        return null;
    }
